...
from data_utils import parse_dialogs_per_response
...
class chatBot(object):
    ...
    def batch_predict(self,S,Q,n):
        ...
        for start in range(0, n, self.batch_size):
            ...
            print(q)
            print(type(q))
            hulio = input(" Stop it right there !! ")'''
    ...
    def converse(self) :
        ...
        while(True) :
            lines.append(line)
            tokenized_line = parse_dialogs_per_response(lines,self.candid2indx)
            if(not tokenized_line) :
                ...
                vec_story , vec_ques , vec_resp = vectorize_data([(story,ques,candid_id)], self.word_idx, self.sentence_size,self.batch_size, self.n_cand, self.memory_size)
...
if __name__ == '__main__':
    ...
    if FLAGS.train:
        print(" I am training")
        josephine = input(" given train !!!")
        chatbot.train()
    else:
        print(" I am testing")
        selena = input(" Proceed To Step #2 !!! ")
        chatbot.test()
